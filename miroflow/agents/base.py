...
class BaseAgent(ABC):
    ...
    USE_PROPAGATE_MODULE_CONFIGS = ("llm", "tools", "prompt")#z 这些也叫代理啊
    USE_TOOLS: bool = True  #z Set to False in subclasses that never call external tools .部分agent无需使用工具
    _instance_counters = {}
    ...
    def __init__(self, cfg: Optional[DictConfig | dict] = None, parent=None):
        ...
        self.llm_client = build_llm_client(cfg=self.cfg.get("llm")) #z main_agent 的llm
        self.prompt_manager = PromptManager(config_path=self.cfg.get("prompt")) #z main_agent llm的配置
        ...
        if self.USE_TOOLS:
            logger.info(f'#z 初始化工具:{self.__class__.__name__}')
            # Parse tool_blacklist from config
            tool_blacklist = self._parse_tool_blacklist(self.cfg.get("tool_blacklist"))
            self.tool_manager = ToolManager(
                cfg=self.cfg.get("tools"), tool_blacklist=tool_blacklist
            ...
            logger.info(f'#z 不初始化工具:{self.__class__.__name__}')
    ...
    async def run(self, ctx: AgentContext) -> AgentContext:
        # logger.info(f'#z start:一次性初始化mcp server：{ctx}')
        await self.post_initialize()
        # logger.info(f'#z end:一次性初始化mcp server：{ctx}') #耗时约3分半钟
        # ret = await self.run_internal(ctx) #z
        # return ret
        try:
            ret = await self.run_internal(ctx)
            # logger.info(f'#z run_internal 看每个代理的返回：\n{ret}')
            # logger.debug(f"#z 每个代理的返回output ctx [{self.__class__.__name__}] : {ret}")
            return ret
        finally:
    ...
    def get_mcp_server_definitions_from_tool_definitions(
        ...
        if tool_definitions and len(tool_definitions) > 0:
            for server in tool_definitions:
                ...
                #z: ✅ 关键：添加此说明以确保 LLM 知道服务器名称
                mcp_server_definitions += f"(Server identifier: '{server['name']}' - use this exact name when calling tools from this server)\n"
                if "tools" in server and len(server["tools"]) > 0:
                    for tool in server["tools"]:
                        # print(f'#z tool = {tool}')
                        mcp_server_definitions += f"\n### Tool name: {tool['name']}\n"
                        #z ✅ 添加说明指导 LLM
                        mcp_server_definitions += f"(Tool must be called with server_name='{server['name']}' and tool_name='{tool['name']}')\n"
                        mcp_server_definitions += (
