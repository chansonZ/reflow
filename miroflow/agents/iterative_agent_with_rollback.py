...
class IterativeAgentWithToolAndRollback(BaseAgent):
    ...
    async def run_internal(self, ctx: AgentContext) -> AgentContext:
        ...
        if ctx.get("message_history") is None:
            tracer.info(f'#z 首次运行，无message_history')
            input_processor_output = await self.input_processor.run(
                AgentContext(**ctx, mcp_server_definitions=self.mcp_server_definitions) #z 约20秒
            )
            tracer.info(f'#z 首次运行，input_processor 输出:{input_processor_output}')
        ...
        tracer.info(f'#z message_history: {message_history}')
        turn_count = 0
        ...
        while max_turns == -1 or turn_count < max_turns:
            ...
            if self.verbose:
                print(f"[Turn {turn_count}]( max_turns={max_turns}) Calling LLM...", flush=True)

            # LLM call (with ContextLimitError fallback)
            try:
                tracer.info(f'#z IterativeAgentWithToolAndRollback satrt:create_message')
                llm_output = await self.llm_client.create_message(
                    system_prompt=system_prompt,
                    message_history=message_history,
                    tool_definitions=self.tool_definitions,
                )
                tracer.info(f'#z IterativeAgentWithToolAndRollback end:create_message')
            except ContextLimitError: # 退出条件 3：ContextLimitError（上下文超长）
                tracer.log(
            ...
            if llm_output.is_invalid:# 退出条件 1：LLM 返回无效响应
                task_failed = True
                break
            ...
            tracer.info(f'#z:satrt:llm_client.extract_tool_calls_info')
            # Tool calls
            tool_and_sub_agent_calls = self.llm_client.extract_tool_calls_info(
                llm_output.raw_response, llm_output.response_text
            )[0]
            tracer.info(f'#z:end:llm_client.extract_tool_calls_info,tool_and_sub_agent_calls={tool_and_sub_agent_calls}')
            ...
            if len(tool_and_sub_agent_calls) == 0:# 退出条件 2（核心）：没有工具调用 → 正常完成
                if (
                    should_rollback
                ...
                consecutive_rollbacks = 0
                duplicate_rollbacks = 0
                tracer.info(f'#z:start:tool_manager.execute_tool_calls_batch')
                (
                    tool_results,
                    tool_calls_exceeded,
                ) = await self.tool_manager.execute_tool_calls_batch(tool_calls)
                tracer.info(f'#z:end:tool_manager.execute_tool_calls_batch')
            ...
            tracer.info(f'#z:satrt:llm_client.get_user_msg_from_tool_call')
            user_msg = self.llm_client.get_user_msg_from_tool_call(
                all_call_results, tool_calls_exceeded
            )
            tracer.info(f'#z:end:llm_client.get_user_msg_from_tool_call user_msg={user_msg}')
            ...
            if _summary_prompt_for_context_check:
                ...
                if not can_continue: # 退出条件 4：主动上下文检查（快接近 token 上限）
                    tracer.log(
        ...
        if max_turns != -1 and turn_count >= max_turns:
            reached_limit = True
        tracer.info(f'#z:IterativeAgentWithToolAndRollback start:output_processor')
        ...
        tracer.info(f'#z:IterativeAgentWithToolAndRollback end:output_processor\n  【{output_processor_result}】')
        tracer.save_agent_states(
            self.name,
            states={
                "message_history": message_history,
