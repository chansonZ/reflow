...
class LLMClientBase(ABC):
    ...
    def __init__(self, cfg: DictConfig):
        ...
        self.strip_think_from_history: bool = self.cfg.get(
            "strip_think_from_history", False #z 看没有配置，默认false
        )
    ...
    async def create_message(
        ...
        response = None
        logger.info(f'#z: 调用模型({self.model_name})  system_prompt = 【{system_prompt}】')
        logger.info(f'#z: 调用模型({self.model_name})  message_history=【{message_history}】  ')
        logger.info(f'#z: 调用模型({self.model_name})  tool_definitions=【{tool_definitions}】')
        logger.info(f'#z: 调用模型({self.model_name})  keep_tool_result=【{keep_tool_result}】')
        ...
        logger.info(f'#z: 调用模型返回({self.model_name})  response_text=【{response_text}】' )
        logger.info(f'#z: 调用模型返回({self.model_name}) is_invalid=【{is_invalid}】' )
        logger.info(f'#z: 调用模型返回({self.model_name}) assistant_message=【{assistant_message}】' )
        logger.info(f'#z: 调用模型返回({self.model_name}) response=【{response}】' )
        return LLMOutput(
