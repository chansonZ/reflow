...
class MiroThinkerVLLMClient(LLMClientBase):
    def __post_init__(self):
        super().__post_init__()

    def _create_client(self, config: DictConfig): #z config  没有使用啊
        """Create configured OpenAI client for MiroThinker via vLLM"""
        print(f'模型配置：{self.cfg.base_url};{self.cfg.api_key}')
    ...
    async def _create_message(
        ...
        """
        Send message to MiroThinker vLLM API.
        :param system_prompt: System prompt string.
        :param messages: Message history list.
        :return: API response object or None (if error).
        """
        logger.debug(
            f" Calling MiroThinker vLLM ({'async' if self.async_client else 'sync'})"
        )
        print(
            f" Calling MiroThinker vLLM ({'async' if self.async_client else 'sync'})"
        )
        # print(f'tools_definitions:\n{tools_definitions}')
        ...
        try:
            ...
            # vLLM-specific params must go through extra_body
            ...
            if self.reasoning_effort and self.reasoning_effort != "":
                extra_body["reasoning_effort"] = self.reasoning_effort
            if extra_body:
                params["extra_body"] = extra_body
            # print(f'开始_create_completion')
            ...
            for length_attempt in range(self._LENGTH_RETRY_MAX + 1):
                ...
                print(f'#z 模型直接返回:_create_completion：response：{response}')
                if (
                    response is None
            ...
            if (
                ...
                logger.debug(f"MiroThinker vLLM Context limit exceeded: {error_str}")
                raise ContextLimitError(f"Context limit exceeded: {error_str}")

            logger.error(
                f"MiroThinker vLLM call failed [{type(e).__name__}]: {error_str}",
                exc_info=True,
            )
            raise e
    ...
    def process_llm_response(self, llm_response) -> tuple[str, bool, dict]:
        """
        Process MiroThinker vLLM response
    ...
    def extract_tool_calls_info(self, llm_response, assistant_response_text):
        """Extract tool call information from MiroThinker vLLM response"""
    ...
    def parse_llm_response(self, llm_response) -> str:
        """Parse MiroThinker vLLM response to get text content"""
        if not llm_response or not llm_response.choices:
