...
def utc_iso(ts: Optional[float] = None, tz_str: Optional[str] = None) -> str:
    """
    根据时区生成 ISO 格式时间
    
    参数:
        ts: Unix时间戳，默认为当前时间
        tz_str: 时区字符串，优先使用此参数，否则从 TZ 环境变量读取
    
    注意: 函数名保持 utc_iso 但实际功能已扩展
    """
    if ts is None:
        ts = time.time()
    ...
    if tz_str == "UTC":
        tz = timezone.utc
    elif tz_str in ["Asia/Shanghai", "Asia/Chongqing"]:
...
class TaskLogFile(BaseModel):
    ...
    #z 
    def get_valid_step_logs(self) -> list[Dict[str, Any]]:
        """Return only step events that should count as frontend-visible steps."""
        valid_types = {"span_start", "span_end"}
        return [
            log
            for log in self.step_logs
            if isinstance(log, dict) and log.get("type") in valid_types
        ]

    def get_valid_step_count(self) -> int:
        """Count only span lifecycle events as effective steps."""
        return len(self.get_valid_step_logs())
...
class TaskTracer:
    ...
    def log(
        ...
        msg: str,
        *,
        level: Literal["DEBUG", "INFO", "WARNING", "ERROR"] = "DEBUG",#"INFO",
